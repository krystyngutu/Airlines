        ...
        if filename.endswith(".json"):
            ...
            with open(filepath, 'r') as file:
                try:
                    ...
                    # Example of extracting key fields (adjust based on your actual JSON structure)
                    combined_data = {
                        'date': price_data.get('date'),
                        'origin': price_data.get('origin'),
                        'destination': price_data.get('destination'),
                        'flight_number': price_data.get('flight_number'),
                        'airline': price_data.get('airline'),
                        'departure_time': price_data.get('departure_time'),
                        'arrival_time': price_data.get('arrival_time'),
                        'duration_minutes': price_data.get('duration_minutes'),
                        'aircraft_type': price_data.get('aircraft_type'),
                        'seats_economy': price_data.get('seats_economy', 0),
                        'seats_business': price_data.get('seats_business', 0),
                        'seats_first': price_data.get('seats_first', 0),
                        'swiss_price': price_data.get('swiss_price'),
                        'lowest_competitor_price': price_data.get('lowest_competitor_price'),
                        'swiss_cheaper': price_data.get('swiss_cheaper', False),
                        'competitor_prices': price_data.get('competitor_prices', [])
                    }
